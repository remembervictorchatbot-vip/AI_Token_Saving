...
All commands are pure stdlib, deterministic, location-independent. Run from
anywhere.
...
toks protect --text "..." --mode code|json|text   # protected zones survive
...
-> optional deterministic pre-flight (ruff/mypy/semgrep/vba-lint) -> LLM
review on trimmed context. Function-mode ~85% input saved on leaf changes
(measured: 85.2% / 47.3%).
...
- Older form: `python -m toks.cli <cmd>` from `scripts/`.

See also: [[Benchmark]], [[Installation]].