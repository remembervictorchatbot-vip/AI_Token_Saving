...
class TestParseManifest(unittest.TestCase):
    ...
    def test_flat_form_expands(self):
        cs = toolaudit.parse_manifest(SAMPLE)
        feishu = next(c for c in cs if c["name"] == "gmail")
...
class TestAudit(unittest.TestCase):
    def test_total_tokens(self):
        ...
        # kdocs: 450+375+400, gmail: 60*300, notion: 12*225, agent-mail: 8*175
    ...
    def test_ranks_by_cost_desc(self):
        ...
        self.assertEqual(res["rows"][0]["name"], "gmail")

    def test_flags_over_threshold_as_review(self):
        res = toolaudit.audit_connectors(SAMPLE)
        self.assertIn("gmail", res["review_candidates"])
    ...
    def test_keep_list_exempts(self):
        res = toolaudit.audit_connectors(SAMPLE, keep=["gmail"])
        self.assertNotIn("gmail", res["review_candidates"])
        self.assertEqual(
            next(r for r in res["rows"] if r["name"] == "gmail")["recommendation"],
            "keep (explicit)",
        )
